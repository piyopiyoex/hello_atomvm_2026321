...
`AtomLGFX.Color` には、RGB565 表示色、RGB888 パレット色、添字色を扱うための補助関数があります。
...
引数には開始時設定のキーワードリストまたはプロパティリストを渡します。  
省略した項目には、ドライバーのビルド時既定値が使われます。

たとえば、この環境では次のような設定で開始できます。必要に応じて変更してください。
...
:ok = AtomLGFX.draw_line(port, 0, 0, 100, 100, 0xFFFF)
:ok = AtomLGFX.draw_rect(port, 10, 10, 50, 30, 0x07E0)
...
この画素バイナリは、通常の RGB565 データをリトルエンディアン 16 ビット語として並べたものです。
